	...
	do
		...
		for k, v in ipairs(weapons.GetList()) do
			...
			if (weapons.IsBasedOn(v.ClassName, "cw_base")) then
				...
				if ITEM then
					HOLSTER_DRAWINFO = ITEM.holsterDrawInfo
...
if (SERVER) then
	...
	local function drawMaterial(exIcon, w, h, name, type)
		...
		if (type == 1) then
			nut.util.drawText(name, sizeGenerate*.24, sizeGenerate*.03, color_white, 1, 1, "nutItemDescFont", 255)
			...
			nut.util.drawText(name, sizeGenerate*.2, sizeGenerate*.23, color_white, 1, 1, "nutItemDescFont", 255)
